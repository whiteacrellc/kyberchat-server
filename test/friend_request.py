#!/usr/bin/env python3
"""Send a friend request via POST /friends/request."""
...
def main():
    parser = argparse.ArgumentParser(description="Send a friend request")
    ...
    parser.add_argument("--friend", required=True, help="Username to send friend request to")
    ...
    print(f"\nSending friend request to '{args.friend}'...")
    payload = json.dumps({"username": args.friend}).encode("utf-8")
    req = urllib.request.Request(
